...
- [x] Write tests for latest-row isolation, exact forecast/anomaly matching, stale exclusion, fallback, empty data, and safe export.
- [x] Run `python -m pytest -q tests/test_station_comparison.py` and confirm collection fails because the module does not exist.
- [x] Implement the minimal comparison module.
- [x] Run `python -m pytest -q tests/test_station_comparison.py` and confirm all tests pass.
- [x] Refactor names and column ordering while keeping the focused tests green.
...
- [x] Add failing source and HTML tests for the new tab, selector, recommendation copy, cards and mobile CSS.
- [x] Run the focused tests and confirm expected failures.
- [x] Add comparison display labels, high-contrast CSS and escaped card HTML.
- [x] Add the sixth tab, two-to-three station selector, recommendation band, grouped chart, 24-hour trend, table and CSV download.
- [x] Run app import and comparison tests until green.
...
- [x] Document the comparison workflow, recommendation basis, stale-data rule and limitations.
- [x] Add the feature to the Dashboard and interview-demo sections.
- [x] Scan README for contradictory claims or wording that implies official guidance.
...
- [x] Run `python run_all.py --mode sample`.
- [x] Run `python src/smoke_test.py`.
- [x] Run `python -m pytest -q`.
- [x] Run `python -m pip check` and `python -m pip_audit --local`.
- [x] Run `git diff --check` and the secret-assignment scan.
- [x] Verify all six tabs, comparison selection, desktop layout, 390px mobile layout, console logs, overflow and raw-code exposure in a real browser.
- [x] Re-run focused tests after any visual defect fix.
